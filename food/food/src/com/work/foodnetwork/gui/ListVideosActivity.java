...
public class ListVideosActivity extends BaseActivity implements OnFullscreenListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			getActionBar().setTitle(getIntent().getStringExtra("CHANEL_NAME"));
		} catch (Exception e) {
		}
	}
	...
	/**
	 * Sets up the layout programatically for the three different states. Portrait, landscape or fullscreen+landscape.
	 * This has to be done programmatically because we handle the orientation changes ourselves in order to get fluent
	 * fullscreen transitions, so the xml layout resources do not get reloaded.
	 */
	private void layout() {
	...
	public void onClickClose(@SuppressWarnings("unused") View view) {
		...
		videoFragment.pause();
		ViewPropertyAnimator animator = videoBox.animate().translationYBy(videoBox.getHeight())
	...
	public void finish() {
		...
		MyApplication.videoEntries = null;
		super.finish();
	}
	...
	public static final class VideoListFragment extends ListFragment {
		...
		static {
			...
			if (MyApplication.videoEntries != null)
				for (MyVideoEntry videoEntry : MyApplication.videoEntries) {
					list.add(new VideoEntry(videoEntry.title, videoEntry.id));
				}
		...
		public void onListItemClick(ListView l, View v, int position, long id) {
			...
			VideoFragment videoFragment = (VideoFragment) getFragmentManager().findFragmentById(
	...
	/**
	 * Adapter for the video list. Manages a set of YouTubeThumbnailViews, including initializing each of them only once
	 * and keeping track of the loader of each one. When the ListFragment gets destroyed it releases all the loaders.
	 */
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (view == null) {
				...
				if (loader == null) {
					...
					MyLog.iGeneral("Video Id: " + entry.videoId);
	...
	public static final class VideoFragment extends YouTubePlayerFragment implements OnInitializedListener {
		...
		public void onInitializationSuccess(Provider provider, YouTubePlayer player, boolean restored) {
			...
			player.setOnFullscreenListener((ListVideosActivity) getActivity());
